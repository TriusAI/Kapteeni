...
| public accuracy (231 items) | **0.537** |

Axes (their formulas):

| Axis | ours | notes |
|---|---|---|
| Intelligence | **30.9** | 3 public tiers, chance-corrected, weights renormalized (judge sealed) |
| Calibration | **53.6** | ECE half only (0.232 top-label); the benchmark also averages a private TVD half |
| Speed | **68.5** | p50 0.38 s / p95 9.48 s on **contended** Strix Halo, ×2 self-hosted adjustment |
| Cost | **42.4** | 597 input tokens/decision × assumed hosted Qwen-4B price ($0.14/M → $0.084/1k decisions); at $0.05/M the axis would be 55.8 |
...
Diagnosis: the frozen backbone's *native* decision readout (raw yes/no
logits at the `answer:` position) beat our trained heads (I 46.4 vs 30.9
on the public half) exactly on the out-of-distribution families — heads
trained on narrow data lose to the backbone itself off-distribution. We
added the native readout (the same forward pass, ~free) as a second
opinion:

| readout | easy | standard | hard | Intelligence | top-label ECE | score |
|---|---:|---:|---:|---:|---:|---:|
| head-only (v0) | 0.979 | 0.472 | 0.387 | 30.9 | 0.232 | 17.9 |
| hybrid w=0.7 *(pre-registered)* | 1.000 | 0.528 | 0.405 | 35.8 | 0.167 | 26.3 |
| hybrid w=0.5 | 1.000 | 0.556 | 0.414 | 37.9 | 0.128 | 30.8 |
| **hybrid w=0.25 (serving default)** | **1.000** | **0.625** | **0.423** | **42.5** | **0.065** | **41.2** |
| verbalizer-only | 0.958 | 0.597 | 0.432 | 40.3 | 0.060 | 36.8 |
...
## P2 (kapteeni-v1): score 65.71 — with asterisks, stated plainly
...
1. **Self-reported public-half run** (no sealed items, no official rank,
   and per repo policy no placement claims either); the sealed set rotates
   and the gap penalty would apply to a sealed measurement we cannot take.
2. Our Intelligence (60.3) is renormalized over the three public tiers
   because the judge tier (146 items) is sealed; any score that folds the
   sealed+judge weight in is not comparable to ours.
3. Calibration is the ECE half only (the benchmark averages a private
...
## Benchmark update during this work (v1.4.1 / v1.4.2)

The benchmark released v1.4.1 and v1.4.2 while the v1.2/v1.2.1 experiments
were running (clone now at `1bcc55e`). Verified facts: the 231 public
items, the 308 sealed set, and the composite formula are **unchanged** —
we reproduced the official arithmetic exactly and `score_bench.py`'s v13
helpers compute the same composite — so all runs of record remain valid
as scored. What changed is the roster of measured systems (93 total, 89
ranked). Repo policy since 2026-09-26: we report our numbers and their
caveats, and make no claims about the leaderboard or placement.
...
- **Sample sizes are small.** All accuracies are on 231 items (easy 48 /
  ...
  0.615 ± 6.3pt, v1 0.710 ± 5.9pt, v1.1 soup 0.697 ± 5.9pt. At this sample
  size, orderings that differ by a few accuracy points are not
  resolvable. Tier-level CIs are wider still (hard: ±9.3pt at v1's 0.469).
- **Small composite gaps are ties.** Differences of a few points between
  any two of our runs (or any two systems on this benchmark) sit inside
  run-to-run variance plus axis assumptions (the Cost axis alone swings
  ±7 points across the stated price range).
...
| | meticulous (v1) | v1.2 | intuit (v1.2.1) |
|---|---:|---:|---:|
| Intelligence | 60.3 | **62.0** | 61.1 |
| top-label ECE -> Calibration | **0.0496** -> 90.1 | 0.2023 -> 59.6 | 0.1196 -> 76.1 |
| **score** | **65.71** | 59.65 | 63.18 |

Gate 3 failed (63.18 < 64.71): the flagship (default) model stays
**-meticulous**, and — as pre-registered — this is the last change gated
on public-half outcomes. (The seed-3 artifacts were later shipped as the
**-intuit** variant with per-traffic guidance rather than discarded; see
the Variants section in the README.)
...
| | meticulous (v1, seed 0) | v1.1 (soup) |
|---|---:|---:|
...
## Run of record (2026-09-24, post double-softmax fix): score 48.2
...
All previous caveats stand (public half, self-reported; judge tier sealed;
ECE-half calibration; cost = stated assumption; w=0.25 public-half-selected,
flagged).
